...
socket.on('gameState', (state) => {
  ...
  if (state.lastPlay) {
    document.getElementById('last-action-box').innerText = 
      `Last action: ${state.lastPlay.player} played ${state.lastPlay.cards.length} card(s) claiming '${state.lastPlay.claim}'`;
  }
});
...
socket.on('gameOver', ({ winner }) => {
  alert(`🏆 Game Over! Winner is ${winner}! XP points updated.`);
